...
interface ADS {
  ...
  room: {
    ...
    const [selectedDateRange, setSelectedDateRange] = useState<[Dayjs, Dayjs]>(
      ...
      const fetchDataAds = async () => {
        ...
        try {
          const response = await axios.get(`${baseUrl}/v0/portal/ads?page=1&size=5`);
  ...
  const {  baseUrl } = authContext;
    return (
      <>
        <Box display="flex" sx={{mt : {
          xs :0,
          lg:10
        }}} >
            <Grid container spacing={20} sx={{justifyContent : {sm : "center" , md : "center" , lg:"center" }}}>
                <Grid item xs={12}  md= {12} lg={12} xl ={6}>
                    <Box sx={{ width: '95%' }}>
                        <Typography sx={{ fontSize: {
                          xs :40 ,
                          md:70
                        }, fontWeight: '800' }} variant="body1" color="initial">
                            Forget Busy Work,
                            Start Next Vacation
                        </Typography>
                        <Typography sx={{ fontSize: {
                          xs : 20 ,
                          md :35
                        }, fontWeight: '200', color: theme.palette.grey[500] }} variant="body1" color="initial">
              ...
              <Button onClick={()=> {
                navigate("/explor" , {state: {
                  range: selectedDateRange,
                  persons: bookingGuestCount,
                }})
                
              }} sx={{mt : 6 , px : 10 , fontSize : 20}} variant="contained" color="primary">
                Explor
              </Button> 
  ...
  // */}
                ...
                <Grid item   xs={6} md ={10} lg={10} xl={6} sx={{display : {
                  
                  xs :"none",
                  lg : "block"
                }}}>
                   
                      <Box sx={{ position:"relative" ,height : "500px" ,border :"1px solid black" , width: "80%" , borderTopLeftRadius :"26%" , borderTopRightRadius : "5%" , 
                          borderBottomLeftRadius:"5%"}}>

                         <img style={{ position: "absolute"  , top : "-10%", left :"-18%",height: "100%", width: "100%", objectFit: "cover" , borderTopLeftRadius :"26%" , borderTopRightRadius : "5%" , 
                          borderBottomLeftRadius:"5%" , 
                         }} src={image} alt="image" />
